...
// 数组
var jobs: Array<string> = ['Facebook', 'Google', 'Apple'];  // 泛型
var jobs: string[] = ['Facebook', 'Google', 'Apple'];

// 枚举 默认情况下，类型的初始值是0，其他项的值是依次递增。
...
## 类
构造函数是当类进行实例化时执行的特殊函数, 必须命名为constructor。 TypeScript每个类只能有一个构造函数。这是违背ES6标准的。ES6中，一个类可以拥有不同参数数量的多个构造函数。
```
Class Person {
  name: string;
  age: number;
...
```
## 箭头函数
=>语法还有一个重要的特性，就是它和环 它的外部代码共享同一个this

## 模板字符串   
模板字符串使用反引号
(1) 可以在模板字符串中使用变量(不必被 使用+来 接字符串);
(2) 支持多行字符串。
